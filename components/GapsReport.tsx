...
const GapsReport: React.FC = () => {
  ...
  const [stats, setStats] = useState({
    ...
    totalGapsAmount: 0,
    negativeGaps: 0,
    negativeGapsAmount: 0,
    positiveGaps: 0,
    positiveGapsAmount: 0
  });
  ...
  const loadGaps = () => {
    ...
    if (saved) {
      ...
      const negativeList = activeGaps.filter((g: Gap) => g.gapAmount < 0);
      const positiveList = activeGaps.filter((g: Gap) => g.gapAmount > 0);

      setStats({
        totalGapsCount: activeGaps.length,
        totalGapsAmount: totalAmount,
        negativeGaps: negativeList.length,
        negativeGapsAmount: negativeList.reduce((acc, curr) => acc + curr.gapAmount, 0),
        positiveGaps: positiveList.length,
        positiveGapsAmount: positiveList.reduce((acc, curr) => acc + curr.gapAmount, 0)
      });
    } else {
      setGaps([]);
      setStats({
    ...
    <>
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-gray-400 text-sm font-medium mt-1">Analyse et suivi de la régularisation des écarts de caisse</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-[2rem] p-6 border border-gray-100 shadow-sm relative group">
          ...
          <div className="flex items-baseline gap-1">
            <span className="text-3xl font-black text-emerald-600">{stats.positiveGapsAmount.toLocaleString()}</span>
            <span className="text-xs font-bold text-gray-400 uppercase ml-1">F ({stats.positiveGaps})</span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[2rem] p-6 border border-gray-100 shadow-sm relative group">
          ...
          <div className="flex items-baseline gap-1">
            <span className="text-3xl font-black text-red-500">{stats.negativeGapsAmount.toLocaleString()}</span>
            <span className="text-xs font-bold text-gray-400 uppercase ml-1">F ({stats.negativeGaps})</span>
          </div>
        </div>
      </div>
              ...
              {filteredGaps.length > 0 ? (
                filteredGaps.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-5 text-xs font-bold text-emerald-600">
                      {item.disbursedAmount ? item.disbursedAmount.toLocaleString() + ' F' : '-'}
                    </td>
                    <td className="px-6 py-5">
                    ...
                    <td className="px-6 py-5 text-right">
                       <div className="flex justify-end gap-1">
                          {item.gapAmount < 0 && item.status === 'En attente' && currentUser?.role !== 'agent commercial' && !(currentUser?.role === 'caissier' && item.type === 'CAISSIER') && (
                            <button 
                          ...
                          <select 
                            ...
                            disabled={currentUser?.role === 'agent commercial' || (currentUser?.role === 'caissier' && item.type === 'CAISSIER') || (item.status && item.status !== 'En attente')}
