  ...
  useEffect(() => {
    const updateState = () => {
      ...
      const defaults: Record<string, string[]> = {
        ...
        'caissier': ['Accueil', 'Membres', 'Alerte Doublons', 'Analyse', 'Suivi des crédits', 'Vente Livrets', 'Versement du jour', 'Gestion Caisse', 'Dépenses administratives', 'Frais & Parts Sociales', 'Déblocage de crédit', 'Journal Global', 'Reçu de caisse', 'Etats des écarts', 'Rapports Financiers', 'Stocks Livrets', 'Annulation Cotisation', 'Notification', 'Guide Pratique'],
      ...
      if (savedPerms) {
        try {
          ...
          // Force inclusion of Annulation Cotisation for caissier if missing
          if (combined['caissier'] && !combined['caissier'].includes('Annulation Cotisation')) {
            combined['caissier'] = [...combined['caissier'], 'Annulation Cotisation'];
          }

          // Force inclusion of Rapport tontine for relevant roles if missing
          ['administrateur', 'directeur', 'agent commercial'].forEach(role => {
            if (combined[role] && !combined[role].includes('Rapport tontine')) {
              combined[role] = [...combined[role], 'Rapport tontine'];
            }
          });
  ...
  const categories: MenuCategory[] = [
    ...
    },
    {
      ...
      items: [
        ...
        { id: 'Versement du jour', label: 'Versement du jour', icon: <Wallet size={20} /> },
        { id: 'Rapport tontine', label: 'Rapport tontine', icon: <FileText size={20} /> },
      ]
    },
    {
